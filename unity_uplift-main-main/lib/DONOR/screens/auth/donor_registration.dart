...
class Donor_RegistrationScreen extends StatefulWidget {
  const Donor_RegistrationScreen({super.key});

  @override
  _Donor_RegistrationScreenState createState() =>
      _Donor_RegistrationScreenState();
}

class _Donor_RegistrationScreenState extends State<Donor_RegistrationScreen> {
              ...
              const Text(
                ...
                Obx(
                  () => ElevatedButton.icon(
                    ...
                    onPressed: isLoading.value ? null : () => _registerDonor(),
                    ...
                    label: Text(
                      isLoading.value ? 'Processing' : 'Register Donor',
...
Future<String?> uploadDocument(File file, BuildContext context) async {
  try {
    ...
    final refrerence = FirebaseStorage.instance
        .ref()
        .child('documents/${basename(file.path)}');

    // Upload the file to Firebase Storage
    final uploadTask = refrerence.putFile(file);
    ...
    String downloadURL = await refrerence.getDownloadURL();

    // Show SnackBar with file name after upload is complete
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Row(
          children: [
